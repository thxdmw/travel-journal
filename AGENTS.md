...
公共层已全部迁完。Vite 已切换为公开站/后台多页 ESM 构建，产物以 hash 命名并由 `app-manifest.json` 接入 Service Worker 缓存升级。公开端「首页」「旅行列表」「旅行详情」「日记列表」「年度回顾」及共享日记卡片已迁到 `frontend/src/public/`；其余页面与编辑器 IIFE 仍由 `frontend/src/entries/` 按原顺序引入：`public-app.js`、`admin-app.js`、`admin/*`、`common/journal-block-editor.js`、`common/custom-cursor.js`、`common/pwa.js`。下一步继续在这两个入口内逐页替换为 SFC。
