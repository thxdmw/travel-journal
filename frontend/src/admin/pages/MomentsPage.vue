...
import type { TripOption } from '@/api/trip'
...
async function loadTrips() {
  try {
    // 轻量选项接口，旅行再多也不会被前 100 条截断
    trips.value = await tripApi.options()
    localStorage.setItem(tripsCacheKey, JSON.stringify(trips.value))
  } catch (error) {
    try { trips.value = JSON.parse(localStorage.getItem(tripsCacheKey) || '[]') as TripOption[] }
    catch { trips.value = [] }
 ...
 * 地图容器必须用函数 ref 收。
 *
 * 这个 div 长在 `v-for="group in grouped"` 里面。字符串 ref 一旦处在 v-for 作用域内，
 * Vue 就把它当成「一组元素」，routeEl.value 拿到的是数组而不是元素——即便同一时刻只有
 * 一天展开、数组里只有一个元素。数组是 truthy，一路传进 simpleMap 的空值检查，最后在
 * 建图那步抛异常，而那里的 catch 是「后台失败就是没有地图，不弹提示」，于是点「看路线」
 * 什么都不发生，也没有任何报错。
 *
 * ref 的类型是手写的 HTMLElement|null，编译期同样看不出来。函数 ref 不受 v-for 影响，
 * 每个节点各调一次，挂载时传元素、卸载时传 null。
 */
function bindRouteEl(el: Element | ComponentPublicInstance | null) {
  routeEl.value = el instanceof HTMLElement ? el : null
}
