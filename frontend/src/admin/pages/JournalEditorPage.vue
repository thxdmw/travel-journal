...
async function unpublish() { await runAction('unpublish', async () => { if (!id.value) return; await enqueue(async () => { try { const draft = await journalApi.unpublish(id.value!, revision.value ?? 0); revision.value = draft.revision ?? null; form.status = 'DRAFT'; dirty.value = false; props.message('日记已撤回') } catch (error) { if (await handleConflict(error)) return; props.fail(error) } }) }) }
...
async function makePreviewLink() { await runAction('link', async () => { if (!await save(true) || !id.value) return; try { const value = await journalApi.createPreviewLink(id.value); previewLink.value = value.url || `${location.origin}/#/preview/${value.token}`; await navigator.clipboard?.writeText(previewLink.value); props.message('预览链接已复制') } catch (error) { props.fail(error) } }) }
...
<template>
  <div v-loading="pageLoading" class="editor-page" element-loading-text="正在打开日记…" @paste="pasted">
    <div class="editor-top"><el-button link class="editor-back" @click="backToTrip">←<em>返回</em></el-button><div class="editor-top-state"><button type="button" class="editor-save-state" :class="`is-${autoSaveState}`" :disabled="autoSaveState !== 'failed'" @click="autoSave">{{ autoSaveLabel || statusLabel(form.status) }}</button><span class="editor-context">{{ contextLine }}</span></div><span class="word-count">{{ wordTotal }} 字</span>
      ...
      <div class="editor-actions"><el-button @click="openTemplate">{{ form.templateId ? '填写模板' : '从模板开始' }}</el-button><template v-if="form.status === 'PUBLISHED'"><el-button @click="openPublished">查看文章</el-button><el-button :loading="busyAction === 'unpublish'" :disabled="busy" @click="unpublish">撤回</el-button><el-button type="primary" :loading="saving" :disabled="!dirty || busy" @click="updatePublished">更新发布</el-button></template><template v-else><el-button :loading="busyAction === 'link'" :disabled="!id || busy" @click="makePreviewLink">预览链接</el-button><el-button type="primary" :loading="busyAction === 'publish'" :disabled="busy" @click="publish">发布日记</el-button></template></div>
      <button type="button" class="editor-more" aria-label="日记信息与更多操作" @click="metaOpen = true">···</button></div>
    ...
    <el-form ref="formRef" :model="form" :rules="rules" class="editor-meta-group editor-meta-form" :class="{ collapsed: metaCollapsed, 'is-open': metaOpen }"><div class="editor-meta-inner"><div class="editor-meta"><el-form-item prop="title"><el-input v-model="form.title" placeholder="日记标题（发布前必填）" /></el-form-item><el-form-item prop="tripId"><el-select v-model="form.tripId" filterable clearable placeholder="所属旅行（可选）"><el-option v-for="item in trips" :key="item.id" :label="item.title" :value="item.id" /></el-select></el-form-item><el-form-item v-if="form.tripId"><el-select v-model="form.tripStopId" clearable placeholder="城市（可选）"><el-option v-for="item in stops" :key="item.id" :label="item.cityName" :value="item.id" /></el-select></el-form-item><el-form-item prop="occurredOn"><el-date-picker v-model="form.occurredOn" :editable="allowTextInput" format="YYYY年MM月DD日" value-format="YYYY-MM-DD" placeholder="发生日期（必填）" @change="occurredOnTouched = true" /></el-form-item></div>
      ...
      <div class="editor-sheet-actions"><el-button @click="openArticlePreview">预览全文</el-button><el-button @click="metaOpen = false; openTemplate()">{{ form.templateId ? '填写模板' : '从模板开始' }}</el-button><template v-if="form.status === 'PUBLISHED'"><el-button @click="openPublished">查看文章</el-button><el-button :loading="busyAction === 'unpublish'" :disabled="busy" @click="unpublish">撤回</el-button><el-button type="primary" :loading="saving" :disabled="!dirty || busy" @click="metaOpen = false; updatePublished()">更新发布</el-button></template><template v-else><el-button :loading="busyAction === 'link'" :disabled="!id || busy" @click="makePreviewLink">预览链接</el-button><el-button type="primary" :loading="busyAction === 'publish'" :disabled="busy" @click="metaOpen = false; publish()">发布日记</el-button></template></div></div></el-form>
