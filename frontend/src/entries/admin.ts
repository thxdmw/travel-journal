...
import '@/styles/journal-block-editor.css'
import '@/styles/journal-editor-mobile.css'
...
const warning=(text:string)=>ElMessage.warning(text)
const error=(text:string)=>ElMessage.error(text)
const info=(text:string)=>ElMessage.info(text)
const fail=(cause:unknown)=>ElMessage.error(cause instanceof Error?cause.message:'操作失败')
const confirm=(text:string)=>ElMessageBox.confirm(text,'请确认',{type:'warning'})
