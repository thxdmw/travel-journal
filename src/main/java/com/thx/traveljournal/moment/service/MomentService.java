...
public class MomentService {
     ...
     */
    public record MomentView(Long id, Long tripId, Long tripStopId, String cityName,
                             OffsetDateTime occurredAt, LocalDate day, String content,
    ...
    public List<MomentView> list(Long tripId, LocalDate day, boolean unsorted) {
        ...
        // 「某一天」按站点时区切，不能按 UTC——否则晚上八点之后写的会掉到第二天
        if (day != null) query.ge(Moment::getOccurredAt, clock.startOfDay(day))
    ...
    public Moment update(Long id, Moment input) {
        ...
        if (input.getOccurredAt() == null) input.setOccurredAt(current.getOccurredAt());
    ...
    public Map<LocalDate, Long> unsortedCountByDay(Long tripId) {
        return mapper.selectList(new LambdaQueryWrapper<Moment>()
                ...
                .stream().collect(Collectors.groupingBy(
                        moment -> clock.dayOf(moment.getOccurredAt()),
                        java.util.TreeMap::new, Collectors.counting()));
    }

    /** 某一天按时间正序的随手记，供整理成日记使用。 */
    public List<Moment> ofDay(Long tripId, LocalDate day) {
        return mapper.selectList(new LambdaQueryWrapper<Moment>()
                        .eq(Moment::getTripId, tripId)
                        .ge(Moment::getOccurredAt, clock.startOfDay(day))
                        .lt(Moment::getOccurredAt, clock.endOfDay(day)))
                .stream().sorted(Comparator.comparing(Moment::getOccurredAt)).toList();
    }
    ...
    /** 标记这些随手记已经被整理进某篇日记。 */
    @Transactional
    public void markSorted(List<Long> momentIds, Long journalId) {
        for (Long id : momentIds) {
            Moment moment = mapper.selectById(id);
            if (moment == null) continue;
            moment.setJournalEntryId(journalId);
            mapper.updateById(moment);
        }
    }
    ...
    private List<MomentView> toViews(List<Moment> moments) {
        ...
        return moments.stream().map(moment -> new MomentView(
                moment.getId(), moment.getTripId(), moment.getTripStopId(),
                moment.getTripStopId() == null ? null : cities.get(moment.getTripStopId()),
                moment.getOccurredAt(), clock.dayOf(moment.getOccurredAt()), moment.getContent(),
