(function () {
  ...
  const Dashboard = {
    ...
    template: `<div><div class="page-head"><div><h2>管理首页</h2><p>整理旅行计划，也记录旅途之后的故事。</p></div><el-button type="primary" @click="$router.push('/trips')">管理旅行</el-button></div>
      ...
      <div class="panel panel-pad"><h3 style="color:var(--tj-primary);font-family:var(--tj-serif)">最近编辑</h3><el-table :data="stats.recent">
        <el-table-column prop="title" label="日记"/><el-table-column prop="occurredOn" label="日期" width="130"/><el-table-column prop="status" label="状态" width="100"/>
  ...
  const Trips = {
    setup() {
      ...
      onMounted(load);
      return { data,loading,dialog,editing,keyword,form,load,open,save };
    },
    template: `<div><div class="page-head"><div><h2>旅行管理</h2><p>从计划到完成，集中整理每一次出发。</p></div><el-button type="primary" @click="open()">新建旅行</el-button></div>
      ...
      <div class="panel-pad"><div v-loading="loading" class="trip-list"><article v-for="item in data" :key="item.id" class="admin-trip-card" @click="$router.push('/trips/'+item.id)">
        <span class="status">{{item.status}}</span><h3>{{item.title}}</h3><p>{{item.summary||'还没有旅行简介'}}</p><footer><span>{{item.startDate}} — {{item.endDate}}</span><el-button link @click.stop="open(item)">编辑</el-button></footer>
      ...
      <el-dialog v-model="dialog" :title="editing?'编辑旅行':'新建旅行'" width="min(680px,92vw)">
        <el-form label-position="top"><el-form-item label="标题"><el-input v-model="form.title"/></el-form-item>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="开始日期"><el-date-picker v-model="form.startDate" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="结束日期"><el-date-picker v-model="form.endDate" value-format="YYYY-MM-DD"/></el-form-item></div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="状态"><el-select v-model="form.status"><el-option v-for="x in ['PLANNING','ONGOING','COMPLETED','ARCHIVED']" :key="x" :value="x"/></el-select></el-form-item><el-form-item label="币种"><el-input v-model="form.defaultCurrency" maxlength="3"/></el-form-item></div>
  ...
  const TripWorkspace = {
    setup() {
      ...
      onMounted(loadAll);
      return {trip,dashboard,stops,itinerary,budget,expenses,journals,active,dialog,editing,stopForm,itemForm,expenseForm,openStop,openItem,openExpense,saveStop,saveItem,saveExpense,remove,saveCategory,removeJournal,router};
    },
    template: `<div v-if="trip"><div class="workspace-head"><span class="back" @click="router.push('/trips')">← 返回</span><div><h2>{{trip.title}}</h2><div class="workspace-meta">{{trip.startDate}} — {{trip.endDate}} · {{trip.status}}</div></div></div>
      ...
      <el-dialog :model-value="dialog==='stop'" :title="editing?'编辑城市':'添加城市'" width="min(650px,92vw)" @closed="editing=null" v-if="dialog==='stop'"><el-form label-position="top"><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="城市"><el-input v-model="stopForm.cityName"/></el-form-item><el-form-item label="国家"><el-input v-model="stopForm.countryName"/></el-form-item></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="纬度"><el-input-number v-model="stopForm.latitude" :precision="6"/></el-form-item><el-form-item label="经度"><el-input-number v-model="stopForm.longitude" :precision="6"/></el-form-item></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="到达日期"><el-date-picker v-model="stopForm.arrivalDate" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="离开日期"><el-date-picker v-model="stopForm.departureDate" value-format="YYYY-MM-DD"/></el-form-item></div><el-form-item label="备注"><el-input v-model="stopForm.note" type="textarea"/></el-form-item></el-form><template #footer><el-button @click="dialog=''">取消</el-button><el-button type="primary" @click="saveStop">保存</el-button></template></el-dialog>
      <el-dialog :model-value="dialog==='item'" :title="editing?'编辑行程':'添加行程'" width="min(650px,92vw)" @closed="editing=null" v-if="dialog==='item'"><el-form label-position="top"><el-form-item label="标题"><el-input v-model="itemForm.title"/></el-form-item><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="日期"><el-date-picker v-model="itemForm.itemDate" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="类型"><el-select v-model="itemForm.type"><el-option v-for="x in ['TRANSPORT','HOTEL','FOOD','ATTRACTION','SHOPPING','ACTIVITY','OTHER']" :key="x" :value="x"/></el-select></el-form-item></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="开始"><el-time-picker v-model="itemForm.startTime" value-format="HH:mm:ss"/></el-form-item><el-form-item label="结束"><el-time-picker v-model="itemForm.endTime" value-format="HH:mm:ss"/></el-form-item></div><el-form-item label="地址"><el-input v-model="itemForm.address"/></el-form-item><el-form-item label="备注"><el-input v-model="itemForm.note" type="textarea"/></el-form-item></el-form><template #footer><el-button @click="dialog=''">取消</el-button><el-button type="primary" @click="saveItem">保存</el-button></template></el-dialog>
      <el-dialog :model-value="dialog==='expense'" :title="editing?'编辑支出':'记录支出'" width="min(600px,92vw)" @closed="editing=null" v-if="dialog==='expense'"><el-form label-position="top"><el-form-item label="说明"><el-input v-model="expenseForm.description"/></el-form-item><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><el-form-item label="日期"><el-date-picker v-model="expenseForm.expenseDate" value-format="YYYY-MM-DD"/></el-form-item><el-form-item label="金额"><el-input-number v-model="expenseForm.amount" :min="0.01" :precision="2"/></el-form-item></div><el-form-item label="分类"><el-select v-model="expenseForm.budgetCategoryId"><el-option v-for="x in budget.categories" :key="x.id" :label="x.name" :value="x.id"/></el-select></el-form-item><el-form-item label="商户"><el-input v-model="expenseForm.merchant"/></el-form-item><el-form-item label="备注"><el-input v-model="expenseForm.note" type="textarea"/></el-form><template #footer><el-button @click="dialog=''">取消</el-button><el-button type="primary" @click="saveExpense">保存</el-button></template></el-dialog>
  ...
  const JournalEditor = {
    setup() {
      ...
      return{form,trips,stops,media,html,id,uploading,saving,fileInput,textarea,save,publish,unpublish,choose,picked,onPaste,insertImage,setCover,removeMedia,router};
    },
    template: `<div class="editor-page"><div class="editor-top"><el-button link @click="router.back()">← 返回</el-button><h2>编辑旅行日记</h2><span class="status">{{form.status}}</span><div class="editor-actions"><el-button :loading="saving" @click="save">保存草稿</el-button><el-button v-if="form.status==='PUBLISHED'" @click="unpublish">撤回</el-button><el-button type="primary" @click="publish">发布日记</el-button></div></div>
      <div class="editor-meta"><el-input v-model="form.title" placeholder="日记标题"/><el-select v-model="form.tripId" placeholder="所属旅行"><el-option v-for="x in trips" :key="x.id" :label="x.title" :value="x.id"/></el-select><el-select v-model="form.tripStopId" clearable placeholder="城市"><el-option v-for="x in stops" :key="x.id" :label="x.cityName" :value="x.id"/></el-select><el-date-picker v-model="form.occurredOn" value-format="YYYY-MM-DD" placeholder="发生日期"/></div>
  ...
  createApp(App).use(router).use(ElementPlus).mount('#admin-app');
})();
