(function(){const{createApp:ee,ref:r,computed:h,onMounted:b,onBeforeUnmount:x,nextTick:T,watch:E}=Vue,f=window.TravelApi.public,L=new URLSearchParams(location.search).has("theme-preview"),R=(e,t)=>window.TravelTheme.apply(e,t);let U=L?"travel-classic":window.TravelTheme.stored(),I=null;function te(e){U=e,I||R(e)}function _(e){I=e,R(e)}function z(){I=null,R(U)}R(U);function ae(e){if(!e)return null;const t=e.replace(/\/display$/,"");return t===e?null:t+"/thumbnail 480w, "+t+"/medium 768w, "+e+" 1280w"}const j={props:["item"],setup(){return{coverSrcset:ae}},template:`
      ...
      </router-link>`};async function O(e,t,a={}){return e?H(e,typeof a=="boolean"?{fit:a}:a,t||[]):null}async function H(e,t,a,l){e.classList.remove("map-load-failed"),e.querySelector(".map-load-message")?.remove();const n=l?{provider:l}:await window.TravelMap.resolveProvider();let i;try{const o=window.TravelTheme?.mapTokens?.()||{};i=await window.TravelMap.create(e,{provider:n.provider,zoom:3,style:o.style})}catch{return ie(e,t,a,n.provider),null}const s=document.createElement("div");s.className="map-zoom-hint",s.textContent="按住 Ctrl + 滚轮缩放地图",e.appendChild(s);const c=o=>{o.ctrlKey&&(o.preventDefault(),o.stopPropagation(),i.zoomBy(o.deltaY<0?1:-1))};e.addEventListener("wheel",c,{passive:!1});const u=[];if((a||[]).filter(o=>Number.isFinite(Number(o.latitude))&&Number.isFinite(Number(o.longitude))&&!(Number(o.latitude)===0&&Number(o.longitude)===0)).forEach((o,m)=>{const y=[Number(o.latitude),Number(o.longitude)];u.push(y);const p=t.route?'<span class="route-marker">'+(m+1)+"</span>":'<span class="city-marker"></span>',v=t.route?[14,14]:[10,10];i.addMarker(y,{html:p,iconAnchor:v,popup:se(o,m,t.route)})}),t.route&&u.length>1){const o=window.TravelTheme?.mapTokens?.()||{};i.setRoute(u,{color:o.color,width:o.width,dashed:!0,animate:!!o.animateRoute})}if(t.fit&&u.length&&i.fitBounds(u,{padding:[30,30],maxZoom:t.maxZoom||(t.route?11:6)}),requestAnimationFrame(()=>i.invalidateSize()),window.ResizeObserver){const o=new ResizeObserver(()=>i.invalidateSize());o.observe(e);const m=i.destroy.bind(i);i.destroy=()=>{o.disconnect(),e.removeEventListener("wheel",c),m()}}return i}function ie(e,t,a,l){if(!e)return;e.classList.add("map-load-failed");const n=document.createElement("div");n.className="map-load-message";const i=l==="OSM"?"OSM":"高德",s=l==="OSM"?"AMAP":"OSM",c=s==="OSM"?"OSM":"高德",u=document.createElement("p");u.textContent=i+"地图加载失败",n.appendChild(u);const g=document.createElement("button");g.type="button",g.className="map-retry-btn",g.textContent="尝试"+c,g.addEventListener("click",()=>{e.classList.remove("map-load-failed"),n.remove(),H(e,t,a,s)}),n.appendChild(g),e.appendChild(n)}function se(e,t,a){const l=document.createElement("div");l.className="travel-map-popup";const n=document.createElement("strong");if(n.textContent=(a?t+1+". ":"")+[e.cityName,e.countryName].filter(Boolean).join(" · "),l.appendChild(n),e.formattedAddress){const s=document.createElement("p");s.textContent=e.formattedAddress,l.appendChild(s)}const i=[];if(e.arrivalDate&&i.push(e.arrivalDate+(e.departureDate?" — "+e.departureDate:"")),e.tripCount!=null&&i.push(e.tripCount+" 次旅行"),e.publishedJournalCount!=null&&i.push(e.publishedJournalCount+" 篇日记"),i.length){const s=document.createElement("small");s.textContent=i.join(" · "),l.appendChild(s)}return(e.trips||[]).slice(0,3).forEach(s=>l.appendChild(B("旅行 · "+s.title,"#/trips/"+encodeURIComponent(s.slug)))),(e.journals||[]).slice(0,4).forEach(s=>l.appendChild(B(s.title,"#/journals/"+encodeURIComponent(s.slug)))),l}function B(e,t){const a=document.createElement("a");return a.textContent=e,a.href=t,a}const N={emits:["change"],setup(e,{emit:t}){const a=r(window.TravelMap?.manualProvider()||"AUTO"),l=r(!0),n=r("");window.TravelMap?.runtime?.().then(u=>{l.value=window.TravelMap?.providerUsable?.("AMAP",u)!==!1}).catch(()=>{l.value=!1});function i(){return window.TravelMap?.resolveProvider?.().then(u=>{a.value==="AUTO"&&(n.value=u.provider)}).catch(()=>{a.value==="AUTO"&&(n.value="")})}a.value==="AUTO"&&i();const s=h(()=>"自动"+(n.value?"（"+(n.value==="AMAP"?"高德":"OSM")+"）":""));function c(u){a.value!==u&&(u==="AMAP"&&!l.value||(a.value=u,window.TravelMap?.setManualProvider(u==="AUTO"?null:u),u==="AUTO"&&(n.value="",i()),t("change")))}return{current:a,select:c,autoLabel:s,amapEnabled:l}},template:`<div class="map-provider-switch" role="group" aria-label="地图 Provider">
    ...
    </div>`},ne={components:{JournalCard:j,MapProviderSwitch:N},setup(){const e=r(null),t=r(null);let a=null,l=0;async function n(){if(!e.value)return;const i=++l;a?.destroy(),a=null;const s=await O(t.value,e.value.cityMarkers||[],!0);i!==l||!t.value?.isConnected?s?.destroy():a=s}return b(async()=>{e.value=await f.home(),await T(),await n()}),x(()=>{l++,a?.destroy(),window.TravelMap?.destroy(t.value)}),{data:e,mapEl:t,renderMap:n}},template:`
      ...
      <div v-else class="loading">正在翻开旅行手记…</div>`},W=document.getElementById("app")?.[Symbol.for("travel-journal.public-pages")];if(!W?.Trips)throw new Error("公开站缺少 Trips SFC 注册");const le=W.Trips,oe={components:{JournalCard:j,MapProviderSwitch:N},setup(){const e=VueRouter.useRoute(),t=r(null),a=r(null);let l=null,n=0;async function i(){if(!t.value)return;const s=++n;l?.destroy(),l=null;const c=await O(a.value,t.value.stops,{fit:!0,route:!0,maxZoom:10});s!==n||!a.value?.isConnected?c?.destroy():l=c}return b(async()=>{t.value=await f.trip(e.params.slug),_(t.value.theme),await T(),await i()}),x(()=>{n++,l?.destroy(),window.TravelMap?.destroy(a.value),z()}),{data:t,mapEl:a,renderMap:i}},template:`
      ...
      </main><div v-else class="loading">正在读取旅行记录…</div>`},re={components:{JournalCard:j},setup(){const e=VueRouter.useRoute(),t=VueRouter.useRouter(),a=r(null),l=r([]),n=r(!1),i=r(e.query.q||""),s=h(()=>e.query.tag||"");async function c(){n.value=!0;try{a.value=await f.journals(1,12,e.query.q||void 0,e.query.tag||void 0)}catch{a.value={items:[]}}finally{n.value=!1}}function u(){const m={};i.value.trim()&&(m.q=i.value.trim()),s.value&&(m.tag=s.value),t.push({path:"/journals",query:m})}function g(m){const y={};i.value.trim()&&(y.q=i.value.trim()),m!==s.value&&(y.tag=m),t.push({path:"/journals",query:y})}function o(){i.value="",t.push({path:"/journals"})}return E(()=>e.query,c),b(async()=>{await c();try{l.value=await f.tags()}catch{l.value=[]}}),{data:a,tags:l,loading:n,keyword:i,activeTag:s,search:u,pickTag:g,reset:o}},template:`<main class="page"><div class="page-title"><span class="eyebrow">STORIES ON THE ROAD</span><h1>旅行日记</h1><p>风景会远去，文字让当时的心情重新回来。</p></div>
    ...
    </main>`},Y={props:{preview:{type:Boolean,default:!1}},components:{MapProviderSwitch:N},setup(e){const t=VueRouter.useRoute(),a=r(null),l=r(!1),n=r(null),i=r(null),s=r(0),c=h(()=>a.value?window.JournalBlocks.render(a.value.contentJson,a.value.media):""),u=h(()=>Math.max(1,Math.ceil(window.JournalBlocks.wordCount(a.value?.contentJson)/500))),g=h(()=>i.value?i.value.items[i.value.index]:null);function o(d,w){i.value={items:d,index:Math.max(0,w)}}function m(d){if(!(d.target instanceof HTMLImageElement)||!d.target.matches(window.JournalMedia.MEDIA_SELECTOR))return;const w=window.JournalMedia.groupOf(d.target);o(w.map(X=>({src:X.src,caption:X.alt||""})),w.indexOf(d.target))}function y(d){if(!i.value)return;const w=i.value.items.length;i.value.index=(i.value.index+d+w)%w}function p(d){i.value&&(d.key==="Escape"?i.value=null:d.key==="ArrowLeft"?y(-1):d.key==="ArrowRight"&&y(1))}function v(){const d=document.documentElement.scrollHeight-window.innerHeight;s.value=d>0?Math.min(100,Math.max(0,window.scrollY/d*100)):0}const k=r(null),D=r(!1),Z=r(-1);let C=null,P=null,q=!1;const A=h(()=>a.value?.route||[]),V=h(()=>A.value[0]?.source==="moment"),me=h(()=>V.value?"这一天走过的路":"这一天的安排"),he=h(()=>D.value?"停止回放":V.value?"▶ 回放这一天":"▶ 依次看一遍");async function G(){if(!A.value.length||!k.value||C)return;const d=await O(k.value,[],{});if(q||!d){d?.destroy();return}C=d,P=window.DayRoute?.render(C,A.value,{source:A.value[0]?.source,onState:w=>{D.value=w.playing,Z.value=w.index}})}function ge(){P?.play()}function $(){q=!0,P?.destroy(),P=null,C?.destroy(),C=null,window.TravelMap?.destroy(k.value)}function ye(){$(),q=!1,G()}const J=[.88,1,1.14,1.3],M=r(Math.min(J.length-1,Math.max(0,Number(localStorage.getItem("travel-journal.reading-scale"))||1))),fe=h(()=>["小","标准","大","特大"][M.value]);function Q(){document.documentElement.style.setProperty("--reading-scale",J[M.value])}function we(d){M.value=Math.min(J.length-1,Math.max(0,M.value+d)),localStorage.setItem("travel-journal.reading-scale",String(M.value)),Q()}return Q(),E(c,()=>T(()=>{window.JournalMedia.teardown(n.value),window.JournalMedia.enhance(n.value)})),b(async()=>{try{a.value=e.preview?await f.preview(t.params.token):await f.journal(t.params.slug)}catch(d){throw l.value=!0,d}_(a.value.theme),window.addEventListener("keydown",p),window.addEventListener("scroll",v,{passive:!0}),T(()=>{v(),window.JournalMedia.enhance(n.value),G()})}),x(()=>{window.JournalMedia.teardown(n.value),window.removeEventListener("keydown",p),window.removeEventListener("scroll",v),$(),z()}),{data:a,article:n,html:c,lightbox:i,current:g,progress:s,readingMinutes:u,preview:e.preview,previewFailed:l,scaleIndex:M,scaleLabel:fe,scaleMax:J.length-1,stepScale:we,routeEl:k,routePoints:A,routeTitle:me,routeIsReal:V,replaying:D,replayIndex:Z,replayLabel:he,toggleReplay:ge,restartRoute:ye,openLightbox:o,openArticleImage:m,stepLightbox:y}},template:`
      ...
      <div v-else class="loading">正在展开日记…</div>`},ce={components:{MapProviderSwitch:N},setup(){const e=r(null),t=r([]),a=r("全部"),l=r("全部"),n=r("全部"),i=r(!1);let s=null,c=0;const u=h(()=>["全部",...new Set(t.value.map(p=>p.countryName).filter(Boolean))]),g=h(()=>["全部",...new Set(t.value.flatMap(p=>p.visitedYears||[]).map(String))].sort((p,v)=>p==="全部"?-1:Number(v)-Number(p))),o=h(()=>{const p=new Map;return t.value.flatMap(v=>v.trips||[]).forEach(v=>p.set(v.slug,v.title)),[{slug:"全部",title:"全部旅行"},...Array.from(p,([v,k])=>({slug:v,title:k}))]}),m=h(()=>t.value.filter(p=>(a.value==="全部"||p.countryName===a.value)&&(l.value==="全部"||(p.visitedYears||[]).includes(Number(l.value)))&&(n.value==="全部"||(p.trips||[]).some(v=>v.slug===n.value))&&(!i.value||p.publishedJournalCount>0)));async function y(){const p=++c;await T(),s&&(s.destroy(),s=null);const v=await O(e.value,m.value,{fit:!0,maxZoom:7});if(p!==c||!e.value?.isConnected){v?.destroy();return}s=v}return E([a,l,n,i],y),b(async()=>{t.value=await f.cities(),await y()}),x(()=>{s?.destroy(),window.TravelMap?.destroy(e.value)}),{mapEl:e,cities:t,country:a,year:l,trip:n,journalOnly:i,countries:u,years:g,trips:o,filtered:m}},template:`<main class="page"><div class="page-title"><span class="eyebrow">MY FOOTPRINTS</span><h1>足迹地图</h1><p>每一个坐标，都连接着一段已经发生的故事。</p></div>
    ...
    </main>`},S="data:image/svg+xml;utf8,"+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="900" height="600"><rect width="900" height="600" fill="#d8cbb4"/><path d="M0 470L230 250l160 150 140-110 370 310H0z" fill="#8da091"/><circle cx="690" cy="150" r="58" fill="#f7e3a1"/></svg>'),ue=["京都的第三个清晨","青城山下的一整天","冰岛公路上的极光","清迈夜市的一碗面","威尼斯的水上巴士","东京深夜的居酒屋"].map((e,t)=>({id:"preview-"+t,slug:"preview",title:e,excerpt:"风景会远去，文字让当时的心情重新回来。这一段还在等待被慢慢写完。",occurredOn:"2026-0"+(t%9+1)+"-12",cityName:["京都","成都","雷克雅未克","清迈","威尼斯","东京"][t],coverUrl:S})),de={schemaVersion:1,blocks:[{id:"preview-day-opener",type:"day-opener",version:1,title:"",data:{city:"成都",dayLabel:"Day 2",date:"2026-08-10",weather:"晴",route:["成都","都江堰","青城山"],metrics:[{value:"21,430",label:"步"},{value:"¥ 420",label:"花费"}]},settings:{}},{id:"preview-chapter",type:"chapter",version:1,title:"",data:{time:"08:30",title:"清晨出发",note:"从成都出发，一路向西"},settings:{}},{id:"preview-heading",type:"heading",version:1,title:"",data:{text:"都江堰的水声",level:2},settings:{}},{id:"preview-paragraph",type:"paragraph",version:1,title:"",data:{text:"站在鱼嘴分水堤上，能听见很远就传来的水声。两千多年前的工程，到现在还在按原来的方式分水。"},settings:{style:"normal",align:"left"}},{id:"preview-quote",type:"quote",version:1,title:"",data:{text:"有些风景，只有慢下来才看得见。",source:"旅途手记"},settings:{}},{id:"preview-callout",type:"callout",version:1,title:"",data:{tone:"tip",icon:"✦",text:"下午三点后人会少很多，适合拍照。"},settings:{}},{id:"preview-location",type:"location-card",version:1,title:"",data:{name:"青城山",address:"成都市都江堰市青城山镇",hours:"08:00–17:30",cost:"80 元",impression:"树荫很多，山路不算陡，适合慢慢走完一整圈。"},settings:{}},{id:"preview-timeline",type:"timeline",version:1,title:"",data:{items:[{time:"09:30",title:"进入山门",description:"买了一份地图，沿着主路上山"},{time:"11:50",title:"到达上清宫",description:"在这里歇脚吃了午饭"},{time:"15:20",title:"下山回到街子古镇",description:"喝了一下午的茶"}]},settings:{}},{id:"preview-stats",type:"stats",version:1,title:"",data:{items:[{value:"18,642",label:"步"},{value:"12.8 km",label:"路程"},{value:"86",label:"张照片"}]},settings:{}},{id:"preview-image",type:"image",version:1,title:"",data:{previewUrl:S,caption:"山间的一刻"},settings:{}},{id:"preview-gallery",type:"gallery",version:1,title:"",data:{previewUrls:[S,S,S],caption:"这一天拍的照片"},settings:{}},{id:"preview-divider",type:"divider",version:1,title:"",data:{},settings:{}},{id:"preview-day-summary",type:"day-summary",version:1,title:"",data:{items:[{icon:"🌟",label:"今天最喜欢",value:"都江堰的水声"},{icon:"💴",label:"今日花费",value:"¥ 420"}]},settings:{}}]},pe=[{order:1,time:"09:00",title:"成都",note:"从市区出发",latitude:30.6598,longitude:104.0633,photos:[]},{order:2,time:"10:30",title:"都江堰",note:"看鱼嘴分水堤",latitude:31.0044,longitude:103.6053,photos:[]},{order:3,time:"12:00",title:"青城山",note:"爬到上清宫",latitude:30.9021,longitude:103.5678,photos:[]},{order:4,time:"17:00",title:"成都",note:"回到市区",latitude:30.6598,longitude:104.0633,photos:[]}],ve=L?[{path:"/:pathMatch(.*)*",component:{components:{JournalCard:j},setup(){const e=new URLSearchParams(location.search).get("scene")||"home",t=["home","journal","map"].includes(e)?e:"home",a=r(null),l=r(null);let n=null,i=null,s=!1;function c(){if(t!=="map")return;const o=window.TravelTheme?.mapTokens?.()||{};n?.setStyle?.(o.style),i?.refreshTheme?.()}function u(){t!=="journal"||!a.value||(window.JournalMedia.teardown(a.value),window.JournalMedia.enhance(a.value))}function g(){c(),u()}return b(async()=>{if(window.addEventListener("travel-theme-applied",g),await T(),t==="journal"&&window.JournalMedia.enhance(a.value),t==="map"){const o=await window.TravelMap.create(l.value,{provider:"OSM",zoom:8,style:window.TravelTheme?.mapTokens?.().style});if(s||!o){o?.destroy();return}n=o,i=window.DayRoute?.render(n,pe,{source:"moment"})}}),x(()=>{s=!0,window.removeEventListener("travel-theme-applied",g),window.JournalMedia.teardown(a.value),i?.destroy(),n?.destroy(),window.TravelMap?.destroy(l.value)}),{scene:t,journalArticle:a,mapEl:l,homeJournals:ue,journalHtml:window.JournalBlocks.render(de,[])}},template:`
      ...
      </main>`}}]:[{path:"/",component:ne},{path:"/trips",component:le},{path:"/trips/:slug",component:oe},{path:"/journals",component:re},{path:"/journals/:slug",component:Y},{path:"/preview/:token",component:Y,props:{preview:!0}},{path:"/years",component:F},{path:"/years/:year",component:F},{path:"/map",component:ce}],K=VueRouter.createRouter({history:VueRouter.createWebHashHistory(),routes:ve,scrollBehavior:()=>({top:0})});ee({setup(){const e=r(!1),t=r({displayName:"旅行者",avatarUrl:null,themeKey:"travel-classic"});E(()=>K.currentRoute.value.fullPath,()=>e.value=!1);function a(s){s&&document.querySelectorAll(s).forEach(c=>{c.classList.remove("tj-preview-highlight"),c.offsetWidth,c.classList.add("tj-preview-highlight"),setTimeout(()=>c.classList.remove("tj-preview-highlight"),900)})}function l(s){s.origin===location.origin&&(s.data?.type==="travel-theme-preview"?R(s.data.theme,{persist:!1}):s.data?.type==="travel-theme-highlight"&&a(s.data.selector))}function n(s){e.value&&!s.target.closest(".public-nav, .mobile-menu")&&(e.value=!1)}function i(s){s.key==="Escape"&&(e.value=!1)}return b(async()=>{if(window.addEventListener("message",l),document.addEventListener("click",n),window.addEventListener("keydown",i),!L)try{t.value=await f.profile(),te(t.value.theme||t.value.themeKey)}catch{}}),x(()=>{window.removeEventListener("message",l),document.removeEventListener("click",n),window.removeEventListener("keydown",i)}),{menu:e,profile:t,isThemePreview:L}},template:`
      ...
      </div>`}).use(K).component("JournalCard",j).mount("#app")})();
