...
添加内容分两层：`＋` 先给 8 个常用区块（`QUICK_BLOCKS`：图片、章节节点、地点卡片、小标题、今日开场、图片组、今日小结、分隔线），正文不在其中——回车就是下一段；完整的 29 种收在「更多内容」后面，带搜索和分类。区块列表中的图片仅作限高缩略展示，避免一张图片占满整个写作区域；完整效果在图片设置弹窗和公开页面中查看。
